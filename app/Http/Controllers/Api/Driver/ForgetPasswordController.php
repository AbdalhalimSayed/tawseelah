...
use App\Http\Requests\Driver\CheckMailRequest;
use App\Http\Requests\Driver\CheckOtpRequest;
use App\Http\Requests\Driver\PasswordResetRequest;
...
class ForgetPasswordController extends Controller
{
    ...
    public function reset_password(PasswordResetRequest $request){
        if (PasswordReset::where("email", $request->email)->where("code", $request->code)->exists()) {
            ...
            $code->driver->password=bcrypt($request->password);
            $code->driver->save();
